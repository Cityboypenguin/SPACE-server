...
type ListMyCommunitiesUseCase interface {
	Execute(ctx context.Context, q repository.PageQuery) ([]*model.Community, int, error)
}
...
func (uc *ListMyCommunitiesInteractor) Execute(ctx context.Context, q repository.PageQuery) ([]*model.Community, int, error) {
	claims, err := authz.RequireAuth(ctx)
	if err != nil {
		return nil, 0, err
	}
	return uc.communityRepo.ListCommunitiesByUserID(ctx, claims.ID, q)
}

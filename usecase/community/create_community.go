...
type CreateCommunityUseCase interface {
	Execute(ctx context.Context, name, description, avatarKey string) (*model.Community, error)
}
...
func (uc *CreateCommunityInteractor) Execute(ctx context.Context, name, description, avatarKey string) (*model.Community, error) {
	claims, err := authz.RequireAuth(ctx)
	if err != nil {
		return nil, err
	}
	return uc.communityRepo.SaveCommunityWithRoom(ctx, name, description, avatarKey, claims.ID)
}

...
// ToggleMaintenance is the resolver for the toggleMaintenance field.
func (r *mutationResolver) ToggleMaintenance(ctx context.Context, enabled bool) (bool, error) {
	if _, err := requireAdminAuth(ctx); err != nil {
		return false, err
	}
	if err := r.MaintenanceRepository.SetMaintenance(ctx, enabled); err != nil {
		return false, err
	}
	return enabled, nil
}
...
// IsMaintenance is the resolver for the isMaintenance field.
func (r *queryResolver) IsMaintenance(ctx context.Context) (bool, error) {
	if _, err := requireAdminAuth(ctx); err != nil {
		return false, err
	}
	return r.MaintenanceRepository.IsMaintenance(ctx)
}
